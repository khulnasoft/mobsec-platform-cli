/*
 * Copyright © 2021-2022 MobSec Inc.
 *
...
import type { Assessment, Severity } from "@mobsec/platform-lib/lib/types";
import {
  Filter,
...
import {
  DEFAULT_LAB_UI_URL,
  assessmentLink,
} from "@mobsec/platform-lib/lib/platform-config";
...
/**
 * Convert MobSec severity to a SARIF notification level.
 */
 ...
 *
 * The data **must** be provided from the query in @mobsec/platform-lib with includeReport true,
...
export async function convertToSarif(
  ...
  for (const finding of report.findings) {
    ...
    // Format MobSec evidence in a Markdown table, when possible.
    ...
    rules.push({
      ...
      help: {
        ...
        text: "MobSec only provides recommendations in a Markdown format.",
        markdown,
      },
    });
  }
  ...
  const run: Run = {
    tool: {
      driver: {
        name: "MobSec",
        informationUri: "https://www.khulnasoft.com/",
...
async function searchLocations(
  ...
  try {
    ...
    errLog(
      "Note: ripgrep is required for line-number identification. On Ubuntu-based distributions use `apt-get install -y ripgrep` before running the MobSec action"
    );
    return [];
  }
